...
<p><a href="test.php">Create event</a> </p>
...
Categorisation:   <select name="categorisation">
  ...
  <option value="racing">Tournament</option>
</select> <br />
...
if(!empty($_POST['title']) && !empty($_POST['description'])) {
	...
	mysql_select_db('register-events') or die("cannot select DB");

	$query=mysql_query("SELECT * FROM events WHERE title='".$title."'");
	$numrows=mysql_num_rows($query);
	if($numrows==0)
	{
	$sql="INSERT INTO events(title,description,location,startdate,enddate,categorisation,tickets) VALUES('$title','$description','$location','$startdate','$enddate','$categorisation','$tickets')";
